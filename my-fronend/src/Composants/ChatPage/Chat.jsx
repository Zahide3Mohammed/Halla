...
import { IconImage, IconTarget, IconTrash, IconUser } from "../../Elementes/Icons.jsx";
...
useEffect(() => {
    ...
    if (activeTab === "groups" && selectedGroup) {
        ...
        channel = echo.private(`chat.${selectedGroup.id}`);
        channel.listen(".message.sent", (e) => {
            if (String(e.message.group_id) === String(selectedGroup.id)) {
                setMessages(prev => {
    ...
    } else if (activeTab === "amis" && selectedFriend) {
        setIsLoadingMessages(true);
        ...
        // فتح قناة خاصة للمستخدم
        channel = echo.private(`chat.${user.id}`);
        channel.listen(".PrivateMessageSent", (e) => {
            if (String(e.message.sender_id) === String(selectedFriend.id) || 
                String(e.message.receiver_id) === String(selectedFriend.id)) {
                setMessages(prev => {
                    if (prev.some(m => m.id === e.message.id)) return prev;
                    return [...prev, e.message];
                });
    ...
    // Cleanup: إغلاق القناة بشكل صحيح وآمن
    return () => {
        if (channel) {
            echo.leave(activeTab === "groups" ? `chat.${selectedGroup?.id}` : `chat.${user.id}`);
        }
    };
}, [selectedGroup?.id, selectedFriend?.id, activeTab, user?.id, token]);
//=============================================================
        ...
        <div className="AuthX_AppContainer_55" style={{"--user-main-color": user?.color || "#6366f1"}}>       
                                ...
                                return (
                                    <div key={group.id} className={`AuthX_GroupItem_55 ${selectedGroup?.id === group.id ? "AuthX_Active_55" : ""}`} onClick={() => setSelectedGroup(group)}>
                                        ...
                                        <div className="AuthX_GroupInfo_55">
                                            <p style={{ fontWeight:'900', margin : '5px' , fontSize :'15px'}} >
                                                {group.name}
                                            </p>
                ...
                <header className="AuthX_ChatHeader_55">
                    {currentActiveTarget && (
                        <div className="AuthX_HeaderProfile_55">
                            ...
                            <div>
                                <h3 className="AuthX_HeaderTitle_55">{currentActiveTarget.name}{currentTitle}</h3>
                                <span className="AuthX_HeaderSubtitle_55"></span>
                            </div>
                        </div>
                    )}
                </header>
                <div className="AuthX_MessagesArea_55">
                    {isLoadingMessages ? (
                        ...
                        (messages || []).map(m => {
                            ...
                            const senderName = m.user ? `${m.user.prenom} ${m.user.nom}` : (m.sender ? `${m.sender.prenom} ${m.sender.nom}` : "Utilisateur");
                                ...
                                <div key={m.id} className={`AuthX_MsgRow_55 ${isMe ? "AuthX_Me_55" : "AuthX_NotMe_55"}`}>
                                    ...
                                    <div className="AuthX_MsgContent_55">                                        
                                        <div className={`AuthX_MsgBubble_55 ${m.isSending ? 'AuthX_Sending_55' : ''}`}>
                                            ...
                                            <span style={{ fontSize: "10px", color: "#666", display: "block", marginBottom: "2px", fontWeight: "bold" }}>
                                                {senderName}
                                            </span>
                                            )}
            ...
            <aside className="AuthX_SidebarRight_55">
                {activeTab === "groups" && selectedGroup ? (
                    <>
                        <div className="AuthX_GoalCard_55">
                            <span className="AuthX_GoalLabel_55"><IconTarget /> SUGGESTION</span>
                ...
                <button 
                    ...
                    <IconTrash /> Supprimer la discussion
                </button>
            </div>
        </div>
    ) : null}
